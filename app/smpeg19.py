...
class CustomTimelineWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Smart FFmpeg call delay timer to avoid spamming processes when moving the mouse
    ...
    def on_preview_ready(self, sec, pixmap):
        ...
        # Check the mouse position right now
        if getattr(self, 'is_hovering', False):
            ...
            # We update the image ONLY if the sniper brought the frame needed for the second!
    ...
    def trigger_sniper(self):
        """ Triggers only after the mouse stops and gently requests one frame. """
    ...
    def mouseMoveEvent(self, event):
        ...
        if hasattr(self, 'preview_widget'):
            ...
            # Priority 1: Instant reading of the finished frame from the disk (no lag!)
            ...
            if not has_disk_thumb:
                ...
                if hasattr(self, 'sniper') and target_sec in self.sniper.cache:
                    ...
                    # Priority #3: Debounced soft launch without lags and process spam
                    if hasattr(self, 'sniper') and self.current_video_path:
                        self.pending_sec = sec
                        self.sniper_timer.start(120) # Wait 120ms of idle time before launching FFmpeg
                    
                    self.preview_widget.update_info(time_str, is_in_trim, hover_ms, None)
                    self.preview_widget.img_label.setPixmap(QPixmap())
                    self.preview_widget.img_label.setText("Generating...")

            # Preview coordinate positioning

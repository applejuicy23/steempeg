...
class SteempegApp:
    def __init__(self):
        ...
        loader = QUiLoader()
        ui_file_path = get_resource_path("smpegui12.ui")
        ...
        self.ui.setWindowTitle("Steempeg v10")
        ...
        logging.info("="*40)
        logging.info("STEEMPEG 10 RUNNING")
        logging.info("="*40)
        ...
        if hasattr(self.ui, 'table_clips'):
            ...
            # Disable selection focus dots (removes ugly white lines)
            self.ui.table_clips.setFocusPolicy(Qt.NoFocus)
            self.ui.table_clips.viewport().setFocusPolicy(Qt.NoFocus)
            # Force remove dotted outline via CSS (kills the white lines completely)
            # Force remove dotted outline via CSS (kills the white lines completely)
        ...
        if hasattr(self.ui, 'btn_about'):
            self.ui.btn_about.clicked.connect(self.show_about_dialog, Qt.UniqueConnection)
        ...
        # 1. Create the player widget and embed it
        self.video_widget = QVideoWidget()
        video_layout = QVBoxLayout(self.ui.video_container)
        video_layout.setContentsMargins(0, 0, 0, 0)
        video_layout.addWidget(self.video_widget)

        # 2. Create player "brains" and audio output
        ...
        # 3. CONNECTING RELIABLE connections
        self.player.errorOccurred.connect(self.on_player_error)
        self.player.mediaStatusChanged.connect(self.on_player_status)
        ...
        # REPLACE THE OLD LOG BUTTON WITH A MENU
    ...
    def toggle_play(self):
        """ Play / Pause toggle with auto-preview generation """
        # If the video is not yet in the player generate a preview
        if not self.player.source().isValid():
            self.generate_and_play_preview()
            return # Exit, the generation function will automatically press Play
    ...
    def generate_and_play_preview(self):
        """ Creates a fast, lightweight H.264 preview using FFmpeg """
        if not hasattr(self.ui, 'table_clips') or self.ui.table_clips.currentRow() < 0:
            return

        # 1. COMPLETE RESET OF THE PLAYER
        self.player.stop()
        ...
        if not all_mpds: return

        ffmpeg_exe = get_resource_path("ffmpeg.exe")
        mpd_path = all_mpds[0] 
        clip_dir = os.path.dirname(mpd_path)
        preview_path = os.path.join(self.cache_dir, "preview.mp4")

        if os.path.exists(preview_path):
            try: os.remove(preview_path)
            except: pass

        self.ui.btn_play.setText("⏳ Loading...")
        self.ui.btn_play.setEnabled(False)
        self.set_status("Generating proxy preview...")
        QApplication.processEvents() 

        try:
            #2. ULTRA-COMPATIBLE TEAM
            cmd = [
                ffmpeg_exe, "-y", 
                "-i", os.path.basename(mpd_path),
                "-vf", "scale=-2:480,setpts=PTS-STARTPTS", 
            ...
            subprocess.run(cmd, shell=False, cwd=clip_dir, creationflags=creation_flags, capture_output=True, text=True)
            
            if os.path.exists(preview_path) and os.path.getsize(preview_path) > 1000:
                print(f"---> Preview ready, size: {os.path.getsize(preview_path)} bytes")
                
                self.ui.video_container.setStyleSheet("") 
                self.video_widget.setStyleSheet("background-color: black;")
                
                
                safe_path = os.path.abspath(preview_path).replace('\\', '/')
                video_url = QUrl(f"file:///{safe_path}")
                print(f"---> Feeding URL to player: {video_url.toString()}")
                ...
                print("❌ Preview file is missing or too small!")
                self.ui.btn_play.setText("▶ Play")
    ...
    def on_player_status(self, status):
        ...
        if status == QMediaPlayer.MediaStatus.LoadedMedia:
            print(f"---> Media Loaded! Duration: {self.player.duration()}ms")
    def skip_backward(self):
        """ Jumps back 10 seconds (10000 ms) """
        self.player.setPosition(max(0, self.player.position() - 10000))

    def skip_forward(self):
        """ Jumps forward 10 seconds (10000 ms) """
    ...
    def update_slider_position(self, position):
        ...
        self.ui.slider_timeline.setValue(position)
        self.ui.slider_timeline.blockSignals(False)
        self.update_time_label()

    def update_slider_duration(self, duration):
        """ Sets the maximum value of the slider based on video length """
        self.ui.slider_timeline.setRange(0, duration)
        self.update_time_label()

    def set_player_position(self, position):
        """ Seeks the video when the user drags the slider """
        self.player.setPosition(position)

    def update_time_label(self):
        """ Formats the milliseconds into MM:SS / MM:SS """
        current = self.player.position() // 1000
        total = self.player.duration() // 1000
        
        current_str = f"{current // 60:02d}:{current % 60:02d}"
        total_str = f"{total // 60:02d}:{total % 60:02d}"
    ...
    def show_about_dialog(self):
        ...
        about_text = (
            "<h3>Steempeg v10.0</h3>"
            "<p><b>Build:</b> 10.0 (Stable)</p>"
            "<p><b>Developer:</b> Emily 🎀 a.k.a applejuicy23</p>"
            "<p><b>GitHub:</b> <a href='https://github.com/applejuicy23/steempeg'>applejuicy23/steempeg</a></p>"

            "<p>A smart, elegant, and fast hardware-accelerated video renderer for Steam Clips.</p>"



            "<p><b>Powered by FFmpeg</p>"
            "<p><b>Steempeg is an unofficial, community-created tool and is not affiliated with, associated with, authorized, endorsed by, or in any way officially connected with Valve Corporation or Steam.</p>"
        )
            ...
            if response.status_code == 200:
                ...
                if match:
                    ...
                    if latest_version > CURRENT_VERSION:
                        ...
                        # Find the download link for the .exe file in GitHub assets
                        download_url = None
                        asset_name = "steempeg_update.exe"
                        ...
                        btn_download = msg.addButton("🚀 Install", QMessageBox.ActionRole)
    ...
    def on_update_downloaded(self, success, filepath, final_asset_name):
        """ Runs when the download finishes. Asks the user about backup and triggers BAT ninja! """
        if success:
            ...
            backup_name = f"{name_only}_backup{ext}"
            
            msg = QMessageBox(self.ui)
            msg.setWindowTitle("Update Ready!")
            msg.setText("The new version has been downloaded.\nDo you want to replace the current version, or keep it as a backup?")
            msg.setIcon(QMessageBox.Question)
            
            btn_delete = msg.addButton("Delete old and replace)", QMessageBox.AcceptRole)
            btn_keep = msg.addButton("📦 Keep backup", QMessageBox.ActionRole)
            msg.exec()
            
            keep_old = (msg.clickedButton() == btn_keep)
            
            # If the names happen to be the same (eg v10 updates v10)
            if base_name == final_asset_name and keep_old:
                final_asset_name = f"{name_only}_new{ext}"

            if keep_old:
                action_logic = f'ren "{base_name}" "{backup_name}"'
            else:
                action_logic = f'del "{base_name}"'
                
            pid = os.getpid() # We take the unique number of the current program
            ...
            {action_logic}
            ren "{os.path.basename(filepath)}" "{final_asset_name}"

            echo Starting new version...
            start "" "{final_asset_name}"
            del "%~f0"
            """
            ...
            # Clear the memory of variables so that the new version does not search for the old _MEI folder
            env = os.environ.copy()
            env.pop('_MEIPASS2', None)
            env.pop('_MEIPASS', None)
            # Clear variable memory so the new version doesn't search the old folder ?mei   
    ...
    def scan_clips(self):
        ...
        # off sorting before adding new data
        ...
        try:
            ...
            for item_name in items:
                ...
                if os.path.isdir(full_path):
                    ...
                    if len(parts) >= 4 and parts[1].isdigit():
                        ...
                        logging.info(f"Found clip: {game_name} | Date: {formatted_date} | Root: {full_path}")
                    else:
                        game_name = item_name
            ...
            # turn on sorting after all data is loaded
    ...
    def update_quality_options(self):
        ...
        self.ui.btn_start.setEnabled(True)
        self.update_final_setup()
        # Reset the player when selecting a new clip
...
if __name__ == "__main__":
    ...
    try:
        ...
        if getattr(window, 'ui', None) is None:
            QMessageBox.critical(None, "Interface Error", "Failed to load smpegui12.ui! Check if the file is located next to the script and that the name matches.")
            sys.exit(1)

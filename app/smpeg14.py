...
class SteempegApp(QObject):
    def __init__(self):
        ...
        self.ui.setWindowTitle("Steempeg v14")
        ...
        logging.info("="*40)
        logging.info("STEEMPEG 14 RUNNING")
        logging.info("="*40)
        ...
        if hasattr(self.ui, 'label_location'):
            ...
            # --- MAGIC SWAP: Replace standard label with our Smart Label ---
        ...
        if right_layout:
            ...
            if controls_index != -1:
                ...
                # ROW 2: The Time Label AND Theater Button (Perfectly centered with dummy widget)
                time_layout = QHBoxLayout()
                
                # --- INVISIBLE DUMMY WIDGET --- 
                self.dummy_theater = QWidget()
                self.dummy_theater.setFixedSize(32, 32)
                self.dummy_theater.setParent(self.player_footer_frame) 
                time_layout.addWidget(self.dummy_theater)
                
                time_layout.addStretch() # Push time to center
                ...
                time_layout.addWidget(self.ui.label_time, alignment=Qt.AlignVCenter)
        ...
        def inject_custom_input(combo_widget, placeholder):
            ...
            # Note: We REMOVED addStretch()! Now the ComboBox will eat all the empty space.
    ...
    def update_ui_from_vlc(self):
        ...
        # --- DASH MANIFEST DURATION FIX ---
        # Steam .mpd chunks sometimes contain more video than the manifest declares.
        # If VLC plays past its estimated length, we dynamically expand the timeline duration.
        if current_time > length and length > 0:
            length = current_time
            
        # If VLC returns 0 for length, try to fallback to our previously parsed MPD duration
        ...
        if is_dead:
            ...
            if length > 0:
                # Reset our new Custom Timeline
                if hasattr(self, 'custom_timeline'):
                    self.custom_timeline.set_position(0)
        ...
        if length > 0 and current_time >= 0:
            ...
            if hasattr(self, 'custom_timeline'):
                ...
                is_dragging = self.custom_timeline.is_dragging
    ...
    def get_all_mpd_paths(self, clip_path):
        ...
        if os.path.exists(clip_path):
            for root, dirs, files in os.walk(clip_path):
                # We specifically look for the original Steam file
                if "session.mpd" in files:
                    original_mpd = os.path.join(root, "session.mpd")
                    # Send it to the doctor to fix Steam's rolling buffer bugs
    ...
    def update_final_setup(self):
        ...
        if "Target File Size" in quality:
            ...
            video_bitrate_display = f"~{val_mbps:.1f} Mbps"
        ...
        # Construct the final detailed text block (Separated FPS and Bitrate!)
        if audio_only:
            detailed_text = (
    ...
    def start_render_thread(self):
        ...
        if "Target File Size" in quality_text:
            ...
            try:
                # Change commas to periods for parsing
...
class CustomTimelineWidget(QWidget):
    """
    Independent Timeline Engine (60 FPS Smooth)
    """
    ...
    def process_60fps_frame(self):
        ...
        if self.is_dragging or self.duration_ms <= 0: return
        
        if now < self.user_seek_lock_time:
            return 
        ...
        if self.is_playing:
            ...
            if abs(drift) > 2000:
                self.visual_ms = predicted_vlc_ms 
            else:
                self.visual_ms += delta_ms * speed_multiplier 
        else:
    ...
    def paintEvent(self, event):
        ...
        if self.duration_ms > 0:
            ...
            painter.fillRect(handle_x, track_y - 4, handle_w, track_height + 8, self.handle_color)

    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            self.is_dragging = True
            self.pause_requested.emit() 
            self.update_visual_math(event.position().x())
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            self.is_dragging = False
            self.user_seek_lock_time = time.time() + 1.0 # Ignore VLC completely for 1 sec!
            self.update_visual_math(event.position().x())
            self.resume_requested.emit() 

    def update_visual_math(self, mouse_x):
    ...
    def force_jump(self, new_position_ms):
        """ Instantly glides the UI and tells VLC to seek """
        if self.duration_ms <= 0: return

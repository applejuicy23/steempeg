...
class SteempegApp(QObject):
    def __init__(self):
        ...
        # "Hide ⬇️" Arch-Shaped Insert Button
        ...
        if hasattr(self.ui, 'settings_tabs'):
            ...
            # 4. STYLING TABS (Source Info, Video settings, etc.) IN THE FORM OF ARCHES
        ...
        self.btn_close_clip.setStyleSheet("""
            ...
            QPushButton:hover { background: #ff4444; border-radius: 4px; }
        """)
    ...
    def close_current_clip(self):
        """ Closes the clip, stops the player, and shows a nice stub. """
        #1. STOP THE VIDEO MUTELY
        if hasattr(self, 'player') and self.player:
            self.player.pause = True #First, turn off the sound/playback
            try:
                # Send a hard command directly to the MPV kernel
                self.player.command('stop') 
            except:
                pass
                
            try:
                # Clear the player's memory (download the file)
                self.player.play("") 
            except:
                pass

        # 2. Deselect the table (WITHOUT TRIGGERS!)
        if hasattr(self.ui, 'table_clips'):
            self.ui.table_clips.blockSignals(True)
            self.ui.table_clips.clearSelection()
            self.ui.table_clips.blockSignals(False)
            
        # 3. Return the blank screen
        if hasattr(self, 'video_stack') and hasattr(self, 'placeholder_frame'):
            self.video_stack.setCurrentWidget(self.placeholder_frame)
            
        #4. Hide the cross and reset the title text
    ...
    def generate_and_play_preview(self):
       ...
       #Recursively search for the file timeline_*.json (using **)
        possible_paths = glob.glob(os.path.join(clip_path, "**", "timeline_*.json"), recursive=True)
        
        # Just in case: if the `timeline_` prefix has somehow changed, search for *any* JSON at all.
        if not possible_paths:
            possible_paths = glob.glob(os.path.join(clip_path, "**", "*.json"), recursive=True)
        
        # Take the first physical file found
        ...
        if hasattr(self, 'custom_timeline'):
            if json_path:
                print(f"JSON FOUND RECURSIVELY: {json_path}")
                
                json_name = os.path.basename(json_path) 
                
                # Extract the video folder name (e.g., fg_730_20260521_204249) from mpd_path.
                ...
                print(f"The JSON file was not found in the folder at all (even recursively): {clip_path}")
                self.custom_timeline.canvas.markers.clear()
                self.custom_timeline.canvas.update()
        ...
        if hasattr(self, 'video_stack'): self.video_stack.setCurrentWidget(self.ui.video_container)
        if hasattr(self, 'btn_close_clip'): self.btn_close_clip.show()
...
class TimelineCanvas(QWidget):
    ...
    def get_icon_pixmap(self, icon_key, is_round):
        """ Gets an icon or GLUES the round numbers (IQ 200) """
        from PySide6.QtGui import QPixmap, QPainter
        from PySide6.QtCore import Qt
        import os
        
        # We return from the cache so as not to overload the processor
        ...
        if is_round:
            ...
            for digit in str(icon_key): 
                ...
                if path and os.path.exists(path):
                    # We shrink the height to 16px so that it fits neatly above the stripe
                    pixmaps.append(QPixmap(path).scaledToHeight(16, Qt.SmoothTransformation))
            
            if not pixmaps: return None
            
            # Glue
            total_width = sum(p.width() for p in pixmaps)
            result = QPixmap(total_width, 16)
            ...
            if path and os.path.exists(path):
                pixmap = QPixmap(path).scaledToHeight(16, Qt.SmoothTransformation)
    ...
    def paintEvent(self, event):
        ...
        if pixels_per_sec < 0.5: step = 300 
        elif pixels_per_sec < 2: step = 60  
        elif pixels_per_sec < 10: step = 10 
        elif pixels_per_sec < 50: step = 5  
        elif pixels_per_sec < 150: step = 1 
        else: step = 0.5 
        ...
        while current_sec <= end_sec:
            ...
            if is_major:
                ...
                painter.drawText(int(x) + 3, int(ruler_y + 8), f"{m}:{s:02d}") 
            else:
        ...
        for marker in getattr(self, 'markers', []):
            ...
            if start_x_vp - 30 <= m_x <= end_x_vp + 30:
                pixmap = self.get_icon_pixmap(marker['icon_key'], marker['is_round'])
                if pixmap:
                    icon_x = int(m_x - pixmap.width() / 2)
                    
                    
                    icon_y = int(track_y - 18) 
                    painter.drawPixmap(icon_x, max(0, icon_y), pixmap)
                    
                    # Tooltip
                    if getattr(self, 'hovered_marker', None) == marker:
                        text = marker['title']
                        if marker['desc']: text += f"\n{marker['desc']}"
                        ...
                        text_rect = fm.boundingRect(QRect(0, 0, 300, 100), flags, text)
                        
                        tt_w = text_rect.width() + 16
                        tt_h = text_rect.height() + 8
                        
                        # Center the tooltip over the icon
                        tt_x = icon_x - tt_w / 2 + pixmap.width() / 2
                        
                        # The tooltip pops up even higher than the icon (so as not to overlap it)
                        tt_y = max(0, icon_y - tt_h - 4) 
                        
                        path = QPainterPath()
                        path.addRoundedRect(QRectF(tt_x, tt_y, tt_w, tt_h), 4, 4)
                        painter.fillPath(path, QColor(20, 20, 20, 230))
                        painter.setPen(QPen(QColor(100, 100, 100), 1))
                        painter.drawPath(path)
                        
                        painter.setPen(QColor(255, 255, 255))
                        painter.drawText(QRectF(tt_x + 8, tt_y + 4, tt_w - 16, tt_h - 8), Qt.AlignLeft | Qt.TextWordWrap, text)
    ...
    def mouseMoveEvent(self, event):
        ...
        for marker in getattr(self, 'markers', []):
            ...
            pw = pix.width() if pix else 16
            
            # Hitbox: icon width, height from the very top (y=0) to y=20
            if abs(x - m_x) <= pw / 2 and 0 <= y <= 20:
                found_marker = marker
                break
        
        # Refresh the screen if the mouse enters or leaves the icon
...
class CustomTimelineWidget(QScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        canvas_h = 32     # Canvas height/divisions (TimelineCanvas)
    ...
    def handle_zoom(self, event):
        ...
        new_zoom = max(1.0, min(old_zoom * zoom_factor, 100.0))
        if new_zoom == old_zoom: return
        self.zoom_level = new_zoom

        # We count the mouse globally
        ...
        # We update ONLY the scroll visibility
        self.update_scrollbar_visibility()

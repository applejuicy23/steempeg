...
class SteempegApp(QObject):
    def __init__(self):
        ...
        self.ui.setWindowTitle("Steempeg v15")
        ...
        logging.info("="*40)
        logging.info("STEEMPEG 15 RUNNING")
        logging.info("="*40)
        ...
        if hasattr(self.ui, 'label_location'):
            ...
            # --- Кeplace standard label with our Smart Label ---
        ...
        if right_layout:
            ...
            if controls_index != -1:
                ...
                # --- BIGGER THEATER BUTTON ---
                self.btn_theater = QPushButton()
                self.btn_theater.setParent(self.player_footer_frame)
                ...
                if os.path.exists(t_icon_path):
                    ...
                    self.btn_theater.setIconSize(QSize(24, 24)) 
                else:
                    self.btn_theater.setText("🎦")
                    self.btn_theater.setStyleSheet("QPushButton { background-color: #ff4444; color: white; border-radius: 20px; }") 
                
                # Connections
                self.btn_theater.clicked.connect(self.toggle_theater_mode)
                self.btn_trim.clicked.connect(self.toggle_trim_state) # Pointing to our new unified function!
                
                # Add to right layout
                rw.addStretch() # Push everything to the right
                rw.addWidget(self.btn_trim, alignment=Qt.AlignVCenter)
                rw.addWidget(self.btn_theater, alignment=Qt.AlignVCenter)
        ...
        # 1. Setup deep debugging logs for VLC Engine (Forward slashes are REQUIRED for libvlc)
        vlc_log_path = os.path.join(self.logs_dir, f"vlc_engine_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log").replace('\\', '/')
        
        # 2. Create the VLC core with verbose file logging
        ...
        # 3. Link VLC directly to your black square container
    ...
    def delete_clip(self, clip_path):
        ...
        if msg.clickedButton() == btn_delete:
            try:
                # 2. Stop VLC playback if the deleted clip is currently playing
    ...
    def eventFilter(self, source, event):
        """ Intercepts right-clicks on the table so they don't select the row. """
    ...
    def set_vlc_volume(self, value):
        """ Passes the volume value (0-100) from the slider to VLC """
        if hasattr(self, 'player') and self.player:
            self.player.audio_set_volume(value)
    def toggle_theater_mode(self):
        """ Safely collapses side and bottom panels and swaps the theater icon """
        self.is_theater = not getattr(self, 'is_theater', False)
        
        # 1. Hide the Left Panel (Clips Library)
        if hasattr(self.ui, 'table_clips'):
        ...
        # 2. Hide the Settings Tabs (Middle Bottom)
        if hasattr(self.ui, 'settings_tabs'):
            self.ui.settings_tabs.setVisible(not self.is_theater)
            
        # 3. Hide the Render Footer (Very Bottom)
        ...
        # 4. Hide the "Browse / Refresh" buttons
        ...
        # 5. --- THE MAGIC SWAP ---
        # Change the button image depending on the mode
        if hasattr(self, 'btn_theater'):
            if self.is_theater:
                ...
                # We are OUT of theater mode. returning to the normal icon
    ...
    def toggle_trim_state(self):
        """ Toggles between Trim mode and Normal mode, updating the button's appearance and icons """
        ...
        if self.custom_timeline.is_trim_mode:
            ...
            # Remove yellow border from video player
            self.ui.video_container.setStyleSheet("background-color: #000000; border: none;")
        else:
            # --- TURN ON TRIM MODE ---
            ...
            # Jump playhead to the start of the new trim section
    ...
    # --- ADVANCED TIMELINE CONTROLLERS ---
    def on_timeline_press(self):
        """ Freezes the video and MUTES audio to protect VLC buffer during drag """
        if hasattr(self, 'player') and self.player:
            self.was_playing_before_drag = self.player.is_playing()
            if self.was_playing_before_drag:
                self.player.pause()
            self.player.audio_set_mute(True) # MUTE ENGINE!

    def on_timeline_seek(self, position_ms):
        """ Commands VLC to jump. The custom widget handles its own anti-snapback math. """
        if hasattr(self, 'player') and self.player:
            self.player.set_time(position_ms)
            self._ignore_vlc_until = time.time() + 1.0 # Ignore VLC lag completely for 1 second

    def on_timeline_release(self):
        """ Restores audio and resumes video playback cleanly """
        if hasattr(self, 'player') and self.player:
            # Restore original mute state from the UI checkbox
            is_muted = False
            if hasattr(self.ui, 'check_mute_audio'):
                is_muted = self.ui.check_mute_audio.isChecked()
            self.player.audio_set_mute(is_muted) # UNMUTE ENGINE!
    ...
    def skip_forward(self):
        ...
        if hasattr(self, 'custom_timeline'):
                self.custom_timeline.set_duration(length)
                
                # Changed from .is_dragging to .drag_state to prevent crash!
                is_dragging = self.custom_timeline.drag_state != 'none'
                is_cooling_down = time.time() < getattr(self, '_ignore_vlc_until', 0)
    ...
    def update_ui_from_vlc(self):
        """ Safely updates the custom timeline from the Qt thread with auto-reset at 00:00 """
        ...
        # --- DASH MANIFEST DURATION ---
        if current_time > length and length > 0:
            length = current_time
            
        if length <= 0 and hasattr(self, 'current_clip_duration_sec') and self.current_clip_duration_sec > 0:
            length = int(self.current_clip_duration_sec * 1000)
        ...
        if length > 0 and current_time >= 0:
            ...
            if hasattr(self, 'custom_timeline'):
                ...
                # BUGFIX: Using getattr prevents any crashes if drag_state is missing!
                is_dragging = getattr(self.custom_timeline, 'drag_state', 'none') != 'none'
                is_cooling_down = time.time() < getattr(self, '_ignore_vlc_until', 0)
                
                # Feed VLC data to our perfectly smooth Timeline Engine
                if not is_dragging and not is_cooling_down:
                    is_playing = (state == vlc.State.Playing)
                    self.custom_timeline.set_vlc_time(current_time, is_playing)
                
                display_time = int(self.custom_timeline.visual_ms)
            else:
                display_time = current_time

            # Calculate the text labels perfectly smoothly
            cur_sec, tot_sec = display_time // 1000, length // 1000
            t_h, t_m, t_s = tot_sec // 3600, (tot_sec % 3600) // 60, tot_sec % 60
            c_h, c_m, c_s = cur_sec // 3600, (cur_sec % 3600) // 60, cur_sec % 60
            
            if t_h > 0: current_str, total_str = f"{c_h:02d}:{c_m:02d}:{c_s:02d}", f"{t_h:02d}:{t_m:02d}:{t_s:02d}"
            else: current_str, total_str = f"{c_m:02d}:{c_s:02d}", f"{t_m:02d}:{t_s:02d}"
            ...
            if hasattr(self, 'custom_timeline') and self.custom_timeline.is_trim_mode:
                if self.custom_timeline.trim_start_ms <= display_time <= self.custom_timeline.trim_end_ms:
                    self.ui.video_container.setStyleSheet("background-color: #000000; border: 2px solid rgba(255, 204, 0, 150);")
                else:
                    self.ui.video_container.setStyleSheet("background-color: #000000; border: none;")
    ...
    def on_update_downloaded(self, success, filepath, final_asset_name):
        ...
        CURRENT_VERSION = "15.0" # The current version should be here
    ...
    def get_all_mpd_paths(self, clip_path):
        """ Recursively finds and FIXES session.mpd files before feeding them to VLC/FFmpeg """
        mpd_paths = []
    ...
    def fix_steam_manifest(self, mpd_path):
        ...
        try:
            ...
            # Fix 1: Update the startNumber for VLC
            content = re.sub(r'startNumber="\d+"', f'startNumber="{min_chunk}"', content)
            
            # Fix 2: ONLY adjust total duration if Steam actually deleted chunks (Rolling Buffer)
            if min_chunk > 1:
    ...
    def recover_orphaned_clip(self, folder_path):
        """ V15 KILLER FEATURE: Generates a valid session.mpd for orphaned chunks! """
        import glob
        import re
        import logging
        import os
        ...
        # Without this file, VLC will try to read empty space and the program will explode!
        ...
        #4. Building Perfect XML
        audio_block = ""
        if has_audio:
            audio_block = f"""
            <AdaptationSet id="1" contentType="audio" segmentAlignment="true">
              <Representation id="1" bandwidth="192000" mimeType="audio/mp4">
                <SegmentTemplate timescale="1000" duration="3000" startNumber="{a_start}" initialization="init-stream1.m4s" media="chunk-stream1-$Number%05d$.m4s" />
              </Representation>
            </AdaptationSet>"""
  ...
  <Period id="0" start="PT0.000S">
    <AdaptationSet id="0" contentType="video" segmentAlignment="true">
      <Representation id="0" bandwidth="10000000" mimeType="video/mp4">
        <SegmentTemplate timescale="1000" duration="3000" startNumber="{v_start}" initialization="init-stream0.m4s" media="chunk-stream0-$Number%05d$.m4s" />
      </Representation>
        ...
        try:
            ...
            logging.info(f"V15 MAGIC: Recovered orphaned clip at {recovered_path}")
    ...
    def update_final_setup(self):
        ...
        if hasattr(self, 'custom_timeline') and self.custom_timeline.is_trim_mode:
            ...
            s_m, s_s = int(start_s // 60), int(start_s % 60)
            e_m, e_s = int(end_s // 60), int(end_s % 60)
            s_h, e_h = int(start_s // 3600), int(end_s // 3600)
    ...
    def calculate_strict_target(self, target_mb, is_lossless=False, is_custom=False):
        ...
        self.custom_target_height = target_h # We save this for FFmpeg!
        # --------------------------------
...
class CustomTimelineWidget(QWidget):
    ...
    def process_60fps_frame(self):
        ...
        # BUGFIX: Only freeze the 60fps animation if we are dragging the PLAYHEAD!
        if self.drag_state == 'playhead' or self.duration_ms <= 0: return
        if now < self.user_seek_lock_time: return 

        if self.is_playing:
            speed_multiplier = 1.0
            time_since_vlc = (now - self.vlc_last_update_time) * 1000.0
            predicted_vlc_ms = self.target_ms + min(time_since_vlc, 500.0)
            drift = predicted_vlc_ms - self.visual_ms
            
            if drift > 150: speed_multiplier = 1.15
            elif drift < -150: speed_multiplier = 0.85
            
            if abs(drift) > 2000: self.visual_ms = predicted_vlc_ms 
            else: self.visual_ms += delta_ms * speed_multiplier 
        else:
            diff = self.target_ms - self.visual_ms
            if abs(diff) > 500: self.visual_ms = self.target_ms
            else: self.visual_ms += diff * 0.2
    ...
    def paintEvent(self, event):
        ...
        # 1. Base Track
        painter.fillRect(0, track_y, width, track_height, self.track_color)
        
        if self.duration_ms <= 0: return
        
        # 2. Played Region
        fill_width = int((self.visual_ms / self.duration_ms) * width)
        painter.fillRect(0, track_y, fill_width, track_height, self.fill_color)
        
        # 3. Trim Region (Yellow Box)
        if self.is_trim_mode:
            start_x = int((self.trim_start_ms / self.duration_ms) * width)
            end_x = int((self.trim_end_ms / self.duration_ms) * width)
            trim_w = end_x - start_x
            
            # Yellow Body
            painter.fillRect(start_x, track_y, trim_w, track_height, self.trim_body_color)
            
            # Left Handle
            painter.fillRect(start_x, track_y - 2, 4, track_height + 4, self.trim_handle_color)
            # Right Handle
            painter.fillRect(end_x - 4, track_y - 2, 4, track_height + 4, self.trim_handle_color)

        # 4. Playback Cursor (White Line)
        handle_w = 4
        handle_x = max(0, min(fill_width - (handle_w // 2), width - handle_w))
        painter.fillRect(handle_x, track_y - 6, handle_w, track_height + 12, self.handle_color)
    ...
    def mouseMoveEvent(self, event):
        if self.drag_state == 'none' or self.duration_ms <= 0: return
        ...
        if self.drag_state == 'playhead':
            ...
            self.trim_start_ms = min(ms, self.trim_end_ms - 1000) # Min 1 second gap
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.drag_state == 'playhead':
            self.user_seek_lock_time = time.time() + 1.0 
...
class VolumeControlWidget(QWidget):
    ...
    def enterEvent(self, event):
        """ Mouse hover -> Expand and show slider """
        self.slider.show()
    ...
    def leaveEvent(self, event):
        """ Mouse leave -> Collapse safely to 44px """
        self.anim.setStartValue(self.width())

...
# swapping QFrame for QWidget so that parent panels don't mess up our background!
class FilterPillButton(QWidget):
    clicked = Signal()
    def __init__(self, parent=None):
        super().__init__(parent)
        
       # Enable CSS support for QWidget and assign a fixed ID
        self.setAttribute(Qt.WA_StyledBackground, True)
        self.setObjectName("FilterPill")
        self.setStyleSheet("""
            QWidget#FilterPill { background-color: #4e4e4e; border: none; border-radius: 14px; }
            QWidget#FilterPill:hover { background-color: #5a5a5a; }
        ...
        # Spacing: 6 (left) + 16 (icon) + 6 (right) = exactly 28px
        layout.setContentsMargins(7, 0, 7, 0)
        layout.setSpacing(0) # Eliminating the invisible gap to avoid the "sausage" effect

        self.icon_lbl = QLabel()
        pix = QPixmap(r"C:\Projects\Steempeg\filter.png").scaled(18, 18, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        self.icon_lbl.setPixmap(pix)

        # The space before the text acts as padding, ensuring it looks good when it slides out
        self.text_lbl = QLabel(" Filters")
        self.text_lbl.setStyleSheet("color: white; font-weight: bold; font-size: 13px; background: transparent;")
        self.text_lbl.setMaximumWidth(0) 
        self.text_lbl.setMinimumWidth(0)

        layout.addWidget(self.icon_lbl)
        layout.addWidget(self.text_lbl)

        self.anim = QPropertyAnimation(self.text_lbl, b"maximumWidth")
        self.anim.setDuration(250) 
        self.anim.setEasingCurve(QEasingCurve.OutCubic) 

    def enterEvent(self, event):
        self.anim.stop()
        self.anim.setStartValue(self.text_lbl.width())
        self.anim.setEndValue(60)# Text width including space
        self.anim.start()
...
class FlowLayout(QLayout):
    """Smart layout: arranges elements in a row, and if they don't fit, moves them to a new line!"""
    def __init__(self): super().__init__(); self.items = []
    def addItem(self, item): self.items.append(item)
    def count(self): return len(self.items)
    def itemAt(self, idx): return self.items[idx] if 0<=idx<len(self.items) else None
    def takeAt(self, idx): return self.items.pop(idx) if 0<=idx<len(self.items) else None
    ...
    def doLayout(self, r, test):
        ...
        for item in self.items:
            ...
            if x + w > r.right() and line_h > 0: x, y, line_h = r.x(), y + line_h + 8, 0
            if not test: item.setGeometry(QRect(QPoint(x, y), item.sizeHint()))
            x += w + 8; line_h = max(line_h, h)
        return y + line_h - r.y()
...
class BlockCombo(QComboBox):
    def __init__(self, items, parent=None):
        ...
        # Auto-correction of single digits
        ...
        """
        self.style_error = self.style_normal.replace("border: 1px solid #333;", "border: 2px solid #ff4444;")
        self.setStyleSheet(self.style_normal)
        self.currentTextChanged.connect(self.validate_text)

    def auto_pad_zero(self):
        """ Converts '1' to '01' upon completion of input. """
    ...
    def validate_text(self, text):
        ...
        if text.lower() in valid_items: self.setStyleSheet(self.style_normal)
        else: self.setStyleSheet(self.style_error)
            
    def is_valid(self):
        return self.currentText().lower() in [self.itemText(i).lower() for i in range(self.count())]
...
from PySide6.QtCore import Qt, QPoint, QTime, QDateTime
...
class FilterMenu(QWidget):
    def __init__(self, parent=None):
        ...
        self.container.setStyleSheet("""
            QFrame { background-color: #252525; border: 1px solid #3d3d3d; border-radius: 16px; }
            QLabel { color: #cccccc; border: none; background: transparent; font-size: 13px; font-weight: bold; font-family: 'Segoe UI'; }
        """)
        ...
        list_style = """
            ...
            QListWidget::item { border-bottom: 1px solid #282828; color: #d1d1d1; padding: 4px 8px; font-size: 13px; font-weight: bold; }
            QListWidget::item:hover { background-color: #303030; }
            QListWidget::item:selected { background-color: #3a2e54; color: #ffffff; }
        """

        # SMART CIRCLES (PILLS) FOR GAMES
        layout.addWidget(QLabel("🎮 Games:"))
        self.games_container = QWidget()
        self.games_layout = FlowLayout()
        self.games_container.setLayout(self.games_layout)
        layout.addWidget(self.games_container)

        # SMART PILLS FOR GUYS
        layout.addWidget(QLabel("📂 Type:"))
        self.types_container = QWidget()
        self.types_layout = FlowLayout()
        self.types_container.setLayout(self.types_layout)
        layout.addWidget(self.types_container)

       # Date and Time Blocks
        layout.addWidget(QLabel("📅 Date:"))
        date_layout = QHBoxLayout()
        self.input_min_date = DateGroup()
        self.input_max_date = DateGroup()
        date_layout.addWidget(QLabel("From:")); date_layout.addWidget(self.input_min_date)
        date_layout.addWidget(QLabel("To:")); date_layout.addWidget(self.input_max_date)
        layout.addLayout(date_layout)

        layout.addWidget(QLabel("⏰ Time of creation:"))
        time_c_layout = QHBoxLayout()
        self.input_min_time = TimeGroup(mode="time")
        self.input_max_time = TimeGroup(mode="time")
        time_c_layout.addWidget(QLabel("From:")); time_c_layout.addWidget(self.input_min_time)
        time_c_layout.addWidget(QLabel("To:")); time_c_layout.addWidget(self.input_max_time)
        layout.addLayout(time_c_layout)

        layout.addWidget(QLabel("⏱ Duration:"))
        dur_layout = QHBoxLayout()
        self.input_min_dur = TimeGroup(mode="duration")
        self.input_max_dur = TimeGroup(mode="duration")
        dur_layout.addWidget(QLabel("Min:")); dur_layout.addWidget(self.input_min_dur)
        dur_layout.addWidget(QLabel("Max:")); dur_layout.addWidget(self.input_max_dur)
        layout.addLayout(dur_layout)

        # GLOBAL SORTING IN THE PANEL
        layout.addWidget(QLabel("🔀 Sort By:"))
        self.combo_sort = QComboBox()
        self.combo_sort.addItems([
            "Default (Don't touch)",
            "Game Name (A - Z)", "Game Name (Z - A)",
            "Type (A - Z)", "Type (Z - A)",
            "Date (Oldest First)", "Date (Newest First)",
            "Duration (Shortest)", "Duration (Longest)"
        ])
        self.combo_sort.setStyleSheet("QComboBox { background: #1e1e1e; color: white; border: 1px solid #333; border-radius: 6px; padding: 4px; font-weight: bold; }")
        self.combo_sort.setCursor(Qt.PointingHandCursor)
        layout.addWidget(self.combo_sort)

        layout.addStretch()

        self.btn_apply = QPushButton("Apply Filters & Sort")
        self.btn_apply.setCursor(Qt.PointingHandCursor)
        self.btn_apply.setStyleSheet("""
            QPushButton { background-color: #b29ae7; color: black; border-radius: 8px; padding: 10px; font-weight: bold; font-size: 14px; border: none; }
            QPushButton:hover { background-color: #cbb5f2; }
        """)
    ...
    def gather_statistics(self, app_window):
        ...
        # Style for our tablets
        pill_style = """
            QPushButton { background: #1e1e1e; border: 1px solid #333; border-radius: 14px; padding: 4px 12px; color: #ccc; font-weight: bold; font-family: 'Segoe UI'; }
            QPushButton:hover { border: 1px solid #555; background: #2a2a2a; }
            QPushButton:checked { background: #b29ae7; color: black; border: 1px solid #cbb5f2; }
        """
        
        # 2. Generating IG circles
        for name, icon in unique_games.items():
            short_name = name[:14] + '...' if len(name) > 14 else name # Truncate if longer than 14 characters
            btn = QPushButton(icon, f" {short_name}")
            btn.setCheckable(True)
            btn.setChecked(True)
            btn.setCursor(Qt.PointingHandCursor)
            btn.setStyleSheet(pill_style)
            btn.setProperty("raw_name", name) # Storing the full name in the button for correct filtering
        ...
        for t_name in sorted(list(unique_types)):
            ...
            btn.setCursor(Qt.PointingHandCursor)
            btn.setStyleSheet(pill_style)
            btn.setProperty("raw_name", t_name) # Hiding the full name
        ...
        years = [str(y) for y in range(min_dt.date().year(), max_dt.date().year() + 1)]
        if not years: years = [str(max_dt.date().year())]
        for grp in (self.input_min_date, self.input_max_date):
            grp.y.blockSignals(True); grp.y.clear(); grp.y.addItems(years); grp.y.blockSignals(False)
            
        self.input_min_date.set_dt(min_dt.date())
        self.input_max_date.set_dt(max_dt.date())
        self.input_min_time.set_t(QTime(0, 0))
        self.input_max_time.set_t(QTime(23, 59))
        if min_sec == 999999: min_sec = 0
        self.input_min_dur.set_sec(min_sec)
        self.input_max_dur.set_sec(max_sec)

        self.btn_apply.setText(f"Apply Filters & Sort ({table.rowCount()})")


    def apply_filters(self):
        """ GOD-CONTROLLER: Mathematical Sort + Perfect Grid Sync! """
        ...
        # Disable stupid default sorting
        ...
        def get_qdate(g):
            if not g.y.is_valid() or not g.m.is_valid() or not g.d.is_valid(): return None
            m_idx = ["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"].index(g.m.currentText().lower()) + 1
            return QDate(int(g.y.currentText()), m_idx, int(g.d.currentText()))

        def get_time_sec(g):
            if not g.h.is_valid() or not g.m.is_valid() or not g.ap.is_valid(): return None
            h, m = int(g.h.currentText()), int(g.m.currentText())
            is_pm = g.ap.currentText().upper() == "PM"
            if is_pm and h < 12: h += 12
            if not is_pm and h == 12: h = 0
            return h * 3600 + m * 60

        def get_dur_sec(g):
            if not g.h.is_valid() or not g.m.is_valid() or not g.s.is_valid(): return None
            return int(g.h.currentText()) * 3600 + int(g.m.currentText()) * 60 + int(g.s.currentText())

        min_date, max_date = get_qdate(self.input_min_date), get_qdate(self.input_max_date)
        min_time, max_time = get_time_sec(self.input_min_time), get_time_sec(self.input_max_time)
        min_dur, max_dur = get_dur_sec(self.input_min_dur), get_dur_sec(self.input_max_dur)

        # 2. EXTRACT DATA ONLY FROM THE TABLE (Do not touch the grid yet!)
        all_data = []
        for row in range(table.rowCount()):
            row_items = [table.takeItem(row, col) for col in range(table.columnCount())]
            all_data.append({ 'table_items': row_items, 'orig_row': row })
        ...
        if sort_idx > 0:
            def get_sort_key(data):
                r = data['table_items']
                if sort_idx in (1, 2): return r[0].text().lower() if r[0] else ""
                if sort_idx in (3, 4): return r[1].text().lower() if r[1] else ""
                if sort_idx in (5, 6): # DATE: convert to timestamp
                ...
                if sort_idx in (7, 8): # DURATION: convert to seconds
            ...
            # Reverse logic: 2 (Z-A), 4 (Z-A), 6 (Newest first), 8 (Longest)
            reverse = sort_idx in (2, 4, 6, 8) 
            all_data.sort(key=get_sort_key, reverse=reverse)

        # 4. PUT EVERYTHING BACK AND FILTER
        visible_count = 0
        for new_row, data in enumerate(all_data):
            row_items = data['table_items']
            ...
            # --- FILTERING LOGIC ---
            show = True
            if show and row_items[0] and row_items[0].text().strip() not in selected_games: show = False
            if show and row_items[1] and row_items[1].text().strip() not in selected_types: show = False

            if show and row_items[2]:
                raw_dt = re.sub(r'\s+', ' ', row_items[2].text().strip())
                dt_obj = None
            ...
            if show and row_items[3]:
                txt = row_items[3].text()
                h = int(re.search(r'(\d+)h', txt).group(1)) if 'h' in txt else 0
                m = int(re.search(r'(\d+)m', txt).group(1)) if 'm' in txt else 0
                s = int(re.search(r'(\d+)s', txt).group(1)) if 's' in txt else 0
                r_dur = h * 3600 + m * 60 + s
            ...
            # Apply visibility to the table row
            table.setRowHidden(new_row, not show)
            if show: visible_count += 1

        self.btn_apply.setText(f"Apply Filters & Sort ({visible_count})")
        self.hide()
        
        # 5. THE MOST IMPORTANT PART: REBUILD THE GRID FROM SCRATCH TO KEEP CUSTOM WIDGETS!
        if hasattr(self.app, 'build_netflix_grid'):
            self.app.build_netflix_grid()
...
class SteempegApp(QObject):
    def __init__(self):
        ...
        if hasattr(self.ui, 'table_clips'):
            ...
            top_pill_layout.addStretch() # Push the filter to the right
            ...
            if header.count() >= 4:
                # SCALING MAGIC
                # Column 0 (Game) — Elastic! Handles all window width changes.
                header.setSectionResizeMode(0, qtw.QHeaderView.Stretch) 
                
                # The remaining columns are interactive (the user can move them with the mouse, but the window does not affect them).
                header.setSectionResizeMode(1, qtw.QHeaderView.Interactive)   
                header.setSectionResizeMode(2, qtw.QHeaderView.Interactive)   
                header.setSectionResizeMode(3, qtw.QHeaderView.Interactive)   
                
                # Set a fixed width for the right columns
                self.ui.table_clips.setColumnWidth(1, 80)  # Type
                self.ui.table_clips.setColumnWidth(2, 130) # Date
                self.ui.table_clips.setColumnWidth(3, 100) # Duration
        ...
        if hasattr(self.ui, 'settings_tabs'):
            ...
            # 1. Create a Hide button
            self.btn_toggle_settings = QPushButton()
            self.btn_toggle_settings.setLayoutDirection(Qt.RightToLeft)
            self.btn_toggle_settings.setIconSize(QSize(18, 18))
            self.btn_toggle_settings.setCursor(Qt.PointingHandCursor)
            
            #2. Make the text bold
            font = self.btn_toggle_settings.font()
            font.setBold(True)
            font.setPointSize(10)
            self.btn_toggle_settings.setFont(font)
            ...
            # 4. STYLING TABS (Source Info, Video settings, etc.) IN THE FORM OF ARCHES + NEON PILLS
            self.ui.settings_tabs.setStyleSheet("""
                ...
                QTabWidget QComboBox:hover, QTabWidget QLineEdit:hover {
                    border: 2px solid #6b5a8e; 
                    background-color: #404040;
                }

                QTabWidget QComboBox:focus, QTabWidget QLineEdit:focus {
                ...
                QTabWidget QComboBox QAbstractItemView {
                    ...
                    border: 2px solid #b29ae7;
                    border-radius: 8px;
                    selection-background-color: #b29ae7;
            ...
            # 4. We write it in the right corner
            self.ui.settings_tabs.setCornerWidget(self.btn_toggle_settings, Qt.TopRightCorner)
            
            self.settings_collapsed = False
            
            # 5. Pure Tuggle Logic

            def update_button_state(hovering=False):
                from PySide6.QtGui import QIcon
                if self.settings_collapsed:
                    self.btn_toggle_settings.setText("Show  ")
                    path = r"C:\Projects\Steempeg\showeye.png" if hovering else r"C:\Projects\Steempeg\hideeye.png"
                else:
                    self.btn_toggle_settings.setText("Hide  ")
                    path = r"C:\Projects\Steempeg\hideeye.png" if hovering else r"C:\Projects\Steempeg\showeye.png"
                self.btn_toggle_settings.setIcon(QIcon(path))

            def toggle_settings():
                self.settings_collapsed = not self.settings_collapsed
                if self.settings_collapsed:
                    bar_h = self.ui.settings_tabs.tabBar().sizeHint().height()
                    new_h = max(bar_h, 28) + 2
                    self.ui.settings_tabs.setMaximumHeight(new_h)
                    
                    # Foolproof lock against upward draft
                    if hasattr(self, 'bottom_v_wrap'):
                        # Forcing the layout to recalculate size with collapsed tabs
                        self.bottom_v_wrap.layout().invalidate()
                        locked_h = self.bottom_v_wrap.sizeHint().height()
                        # Prevent the box from growing beyond this size! The splitter physically won't be able to pull it upwards!
                        self.bottom_v_wrap.setMaximumHeight(locked_h)
                    else:
                        locked_h = new_h

                    # Push the splitter down
                    if hasattr(self, 'main_v_splitter'):
                        self.main_v_splitter.setSizes([10000, locked_h])
                else:
                    # Allow dragging anywhere again
                    if hasattr(self, 'bottom_v_wrap'):
                        self.bottom_v_wrap.setMaximumHeight(16777215)
                        
                    self.ui.settings_tabs.setMaximumHeight(16777215)
                    
                    # Returning the splitter to the default position
                    if hasattr(self, 'main_v_splitter'):
                        self.main_v_splitter.setSizes([750, 250])
                        
                update_button_state(hovering=False)

            # Isolate imports via the qtc alias to avoid the UnboundLocalError
            import PySide6.QtCore as qtc

            class HoverFilter(qtc.QObject):
                def eventFilter(self, obj, event):
                    if event.type() == qtc.QEvent.Type.Enter:
                        update_button_state(hovering=True)
                    elif event.type() == qtc.QEvent.Type.Leave:
                        update_button_state(hovering=False)
                    return False

            self.btn_hover_filter = HoverFilter(self)
            self.btn_toggle_settings.installEventFilter(self.btn_hover_filter)
            
            # We set the correct text and icon for the button on first launch
            update_button_state(hovering=False)

            self.btn_toggle_settings.clicked.connect(toggle_settings)
        ...
        if right_layout:
            ...
            if controls_index != -1:
                ...
                rw.addWidget(self.trim_tools_pill, alignment=Qt.AlignVCenter) # This pill will shift the marker to the left upon appearance!
                ...
                bottom_v_layout.setContentsMargins(0, 0, 0, 0)
                bottom_v_layout.setSpacing(right_layout.spacing())

                # 5. Carefully arrange the components into two boxes.
                put_in_bottom = False
                for item in all_items:
                   # As soon as you come across the tabs, flip the switch down
                    if item.widget() == getattr(self.ui, 'settings_tabs', None):
                        put_in_bottom = True
    ...
    def eventFilter(self, source, event):
        ...
        # 1. Check if the event happened inside the clips table...
        if hasattr(self.ui, 'table_clips') and source == self.ui.table_clips.viewport():
            ...
            if event.type() == QEvent.Type.MouseButtonPress:
                ...
                if event.button() == Qt.RightButton:
                    ...
                    # Return True to block Qt from selecting the row!
                    return True 
    ...
    def show_filter_menu(self):
        ...
        if not hasattr(self, 'filter_menu') or not hasattr(self, 'btn_filter_pill'): return
        
        current_rows = self.ui.table_clips.rowCount()
        
        if not hasattr(self.filter_menu, 'last_row_count') or self.filter_menu.last_row_count != current_rows:
            self.filter_menu.gather_statistics(self)
            self.filter_menu.last_row_count = current_rows 
    ...
    def toggle_fullscreen(self):
        """ Completely isolates the video container with Anti-Spam Lock """
        ...
        if self.is_fullscreen:
            ...
            # Armor-piercing memory protection
            ...
            # WE ARE NO LONGER LOSING ALTITUDE; OTHERWISE, THE PANEL WILL BLOW UP THE MOCK-UP
            from PySide6.QtWidgets import QSizePolicy
            self.player_footer_frame.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Maximum)
            ...
            # The timer will give the interface 50 ms to redraw, and then snap the video into the cell
            QTimer.singleShot(50, snap_to_cage)

            # Restoring buttons to the upper splitter box
            ...
            if getattr(self, 'window_maximized_before', False):
                ...
                # 2. Returning to Maximum
                self.ui.showMaximized()
                
                # STRICTLY SET THE MINIMUM SIZE (1000x600)
                self.ui.setMinimumSize(1000, 650)
                
                self.needs_geometry_restore = True
            else:
                self.ui.showNormal()
                # setting strict parameters here
    ...
    def scan_clips(self):
        ...
        try:
            ...
            for full_path in sorted_folders:
                ...
                item_duration.setTextAlignment(Qt.AlignLeft | Qt.AlignVCenter) 
    ...
    def add_user_marker(self):
        ...
        for m in markers_list:
            if m.get('time_ms', -1) == current_time: return 
...
<MPD xmlns="urn:mpeg:dash:schema:mpd:2011" profiles="urn:mpeg:dash:profile:isoff-live:2011" type="static" mediaPresentationDuration="PT{duration_sec}S">
  <Period id="0" start="PT0.000S">
    ...
    def get_clip_size_and_duration(self, clip_path, mpd_content):
        ...
        if time_match:
            # Extracting "raw" data (this may contain 80 minutes and 0 hours)
            raw_hours = int(time_match.group(1)) if time_match.group(1) else 0
            raw_minutes = int(time_match.group(2)) if time_match.group(2) else 0
            raw_seconds = float(time_match.group(3)) if time_match.group(3) else 0.0

            # Convert EVERYTHING into seconds
            self.current_clip_duration_sec = (raw_hours * 3600) + (raw_minutes * 60) + raw_seconds
            
            # Smart Time Mathematics
            total_sec = int(self.current_clip_duration_sec)
            real_hours = total_sec // 3600
            real_minutes = (total_sec % 3600) // 60
            real_seconds = total_sec % 60
            
            # Formatting: if greater than an hour, display H:MM:SS. If less, display MM:SS
            if real_hours > 0:
                duration_str = f"{real_hours}:{real_minutes:02d}:{real_seconds:02d}"
            else:
                duration_str = f"{real_minutes:02d}:{real_seconds:02d}"
    ...
    def sync_grid_to_table(self):
        """ Keeps the Grid perfectly synced with Table changes without destroying custom widgets! """
        if not hasattr(self, 'grid_clips') or not hasattr(self.ui, 'table_clips'): return
        
        # Simply rebuild the entire grid to mirror the current table state!
        self.build_netflix_grid()
        table = self.ui.table_clips
        
        grid.blockSignals(True)
        
        # 1. Extract all tiles from the screen into temporary memory.
        items_dict = {}
        while grid.count() > 0:
            item = grid.takeItem(0)
            items_dict[item.data(Qt.UserRole)] = item
            
        # 2. Insert them back in EXACTLY the same order they currently appear in the table!
        for row in range(table.rowCount()):
            t_item = table.item(row, 0)
            if t_item:
                uid = t_item.data(Qt.UserRole)
                if uid in items_dict:
                    grid.addItem(items_dict[uid])
                    # If the row is hidden by a filter, hide the tile as well
                    items_dict[uid].setHidden(table.isRowHidden(row))
    ...
    def on_render_finished(self, success, error_msg, output_file):
        ...
        if success:
            ...
            # --- LEFT SIDE: Sad Image ---
            pic_label = QLabel()
            pixmap = QPixmap(r"C:\Projects\Steempeg\saderror.png")
...
class ClipCard(qtw.QWidget):
    def __init__(self, title, date_str, badge_text, thumb_path, icon_path, row_idx, parent=None):
        ...
        if thumb_path and os.path.exists(thumb_path):
            self.thumb_label.setPixmap(qtg.QPixmap(thumb_path))
...
class PreviewSniperWorker(QThread):
    ...
    def request_frame(self, mpd_path, hover_sec):
        ...
        if self.video_path != mpd_path:
            ...
            # If the video is new, the radar starts at 0 seconds
            self.bg_anchor = 0
    ...
    def run(self):
        ...
        while True:
            ...
            if self.target_sec != -1 and self.target_sec != last_serviced:
                # 1. The mouse has moved!
                sec = self.target_sec
                is_background = False
                
                # Instantly reset the Radar, now the epicenter = the cursor location!
                self.bg_anchor = self.target_sec
                self.bg_radius = self.interval
                self.bg_left_done = False
                self.bg_right_done = False
            else:
                # 2. BACKGROUND MODE (Radial Search)
                sec = -1
                
                while not (self.bg_left_done and self.bg_right_done):
                    # If the user moved the mouse while we were searching, we interrupt the background search!
                    ...
                    if not self.bg_left_done:
                        ...
                        if candidate >= 0:
                            ...
                            self.bg_left_done = True # We're stuck at the very beginning (0 sec)
                            
                    # Expanding the radar circles!
                    self.bg_radius += self.interval
                
                # If the radar didn’t find any voids (everything is loaded) - sleep!
                if sec == -1:
                    if self.target_sec == last_serviced:
                        self.msleep(100)
                    continue
            ...
            try:
                ...
                for frame in container.decode(stream):
                    ...
                    if not is_background and self.target_sec == sec:
                ...
                if is_background:
                    direction = "👉" if self.bg_side == "right" else "👈"
                    print(f"[RADAR {direction}] Silently loaded {sec}s")
                else:
                    print(f"[PRIORITY TARGET] Delivered {sec}s to UI!")
                    last_serviced = sec
...
class TimelineCanvas(QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        if getattr(self, 'hovered_marker', None):
            if event.button() == Qt.RightButton:
                # Open the delete/edit menu (Right-click)
                if self.hovered_marker.get('icon_key') == 'usermarker':
    ...
    def show_marker_context_menu(self, pos, marker):
        ...
        menu = QMenu(self)
        menu.setStyleSheet("""
            QMenu { background-color: #2d2d2d; color: white; border: 1px solid #444; border-radius: 4px; }
            QMenu::item { padding: 6px 20px 6px 20px; font-weight: bold; }
            QMenu::item:selected { background-color: #555; }
        """)
...
class CustomTimelineWidget(QScrollArea):
    ...
    @property
    def current_video_path(self): return self.canvas.current_video_path
    @current_video_path.setter
    def current_video_path(self, val): self.canvas.current_video_path = val
...
class SmartSliderFilter(QObject):
    def eventFilter(self, obj, event):
        if event.type() == QEvent.Type.MouseButtonPress and event.button() == Qt.LeftButton:
            ...
            width = obj.width()
            if width > 0:
                # 1. Calculate the new value based on click position
                val = obj.minimum() + ((obj.maximum() - obj.minimum()) * x) / width
                obj.setValue(int(val))
                
                # 2. Update the player instantly
                if hasattr(obj, 'sliderMoved'): obj.sliderMoved.emit(int(val))
                if hasattr(obj, 'valueChanged'): obj.valueChanged.emit(int(val))
                
                # 🔥 THE MAGIC FIX: Return False instead of True!
                # By returning False, we let the click pass through to the slider.
                # Since we just teleported the handle right under the mouse cursor,
                # Qt will grab it automatically and allow you to drag it smoothly!
                return False 

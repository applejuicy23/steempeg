...
class SteempegApp(QObject):
    def __init__(self):
        ...
        self.ui.setWindowTitle("Steempeg v13")
        ...
        logging.info("="*40)
        logging.info("STEEMPEG 13 RUNNING")
        logging.info("="*40)
        ...
        if right_layout:
            ...
            if controls_index != -1:
                ...
                # ROW 2: The Time Label (Centered above buttons)
                self.ui.label_time.setParent(self.player_footer_frame)
                self.ui.label_time.setAlignment(Qt.AlignCenter)
                self.ui.label_time.setStyleSheet("color: #cccccc; font-size: 13px; font-weight: bold; margin-top: 5px; background: transparent;")
                v_layout.addWidget(self.ui.label_time)
                ...
                # Connect the timeline to our new advanced playback controllers
...
class CustomTimelineWidget(QWidget):
    """
    Tim lin dfijklnvbbjknlojklñnvbnjkljklbnñn iojkmfgxcbnokmjlmghjcjdfgbzghcnm
    """
    ...
    def process_60fps_frame(self):
        ...
        if now < self.user_seek_lock_time:
            return # UI is beautifully frozen waiting for VLC to catch up after a click

        if self.is_playing:
            speed_multiplier = 1.0
            
            # Predict true VLC time since its last stuttery update
            ...
            # Speed up or slow down by 15% to sync invisibly
            if drift > 150: speed_multiplier = 1.15
            elif drift < -150: speed_multiplier = 0.85
            
            if abs(drift) > 2000:
                self.visual_ms = predicted_vlc_ms # Hard snap only on massive engine lag
            else:
                self.visual_ms += delta_ms * speed_multiplier # PERFECT LINEAR MOVEMENT
        else:
    ...
    def force_jump(self, new_position_ms):
        """ Instantly glides the UI and tells VLC to seek, applying the anti-snapback lock """
        if self.duration_ms <= 0: return
        
        self.visual_ms = max(0.0, min(float(new_position_ms), float(self.duration_ms)))
        self.target_ms = self.visual_ms 
        self.user_seek_lock_time = time.time() + 1.0 # Lock VLC lag for 1 second!

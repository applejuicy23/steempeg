...
class MPVWrapper(QWidget):
    def __init__(self, parent=None):
        ...
        self.mpv_screen.setStyleSheet("background-color: black;")
    ...
    def resizeEvent(self, event):
        ...
        self.mpv_screen.setGeometry(x, y, video_w, video_h)
        
     
        self.top_line.setGeometry(x - border, y - border, video_w + (border * 2), border)
        self.bottom_line.setGeometry(x - border, y + video_h, video_w + (border * 2), border)
        self.left_line.setGeometry(x - border, y, border, video_h)
        self.right_line.setGeometry(x + video_w, y, border, video_h)
        ...
        if getattr(self, 'hud_reference', None) and self.hud_reference.parent() == self:
            ...
            # Center the panel, with a 30px offset from the bottom.
...
class SteempegApp(QObject):
    def __init__(self):
        ...
        self.aspect_frame.setStyleSheet("background-color: #000000; border: 3px solid transparent; border-radius: 6px;")
    ...
    def toggle_fullscreen(self):
        ...
        if self.is_fullscreen:
            ...
            # Giving the wrapper a direct link to the panel!
            if hasattr(self, 'video_wrapper'):
                self.video_wrapper.hud_reference = self.player_footer_frame
            ...
            # No attempts to restore the Maximized state, to avoid breaking Windows animations
            self.ui.showNormal()
...
class SpeedControlWidget(QWidget):
    ...
    def update_text(self, val):
        """ Dynamically swaps text and generates icons based on speed (0.1 to 5.0) """
        speed = val / 10.0
        speed_str = f"{speed:.1f}"
        
        self.lbl_percent.setText(f"x{speed_str}")
        
        # Launching the Icon Generator!
        icon = self.get_dynamic_icon(speed_str)
        if not icon.isNull():
            self.btn_icon.setIcon(icon)
        else:
            # Fallback option in case image files are missing
            self.btn_icon.setText(f"x{speed_str}")

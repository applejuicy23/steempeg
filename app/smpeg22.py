...
class SteempegApp(QObject):
    def __init__(self):
        ...
        if hasattr(self.ui, 'table_clips'):
            ...
            # UI HIJACK 2.0: CONTAINER & TOGGLE BUTTONS
            import PySide6.QtWidgets as qtw
            import PySide6.QtCore as qtc

            # 1. Creating a luxurious circular container for the library
            self.library_container = qtw.QFrame()
            self.library_container.setStyleSheet("""
                QFrame { background-color: #2d2d2d; border: 1px solid #353535; border-radius: 12px; }
            """)
            lib_layout = qtw.QVBoxLayout(self.library_container)
            lib_layout.setContentsMargins(12, 12, 12, 12)
            lib_layout.setSpacing(10)

            # 2. Mode Switches (List / Grid)
            toggle_layout = qtw.QHBoxLayout()
            toggle_layout.setContentsMargins(0, 0, 0, 0)

            self.btn_view_list = qtw.QPushButton("🗄️ List")
            self.btn_view_grid = qtw.QPushButton("🔲 Grid")
            
            # Active and Inactive Button Styles
            self.toggle_style_active = "background-color: #b29ae7; color: black; border-radius: 6px; padding: 4px 16px; font-weight: bold; font-size: 12px; border: none;"
            self.toggle_style_inactive = "background-color: #1e1e1e; color: #aaaaaa; border-radius: 6px; padding: 4px 16px; font-weight: bold; font-size: 12px; border: 1px solid #333;"
            ...
            toggle_layout.addStretch() # Push the buttons to the left so they sit neatly along the edge.

            lib_layout.addLayout(toggle_layout)

            # 3. Creating Our Netflix Grid
            ...
            self.grid_clips.setStyleSheet("""
                ...
                QListWidget::item { border-radius: 12px; background-color: transparent; }
                QListWidget::item:selected { background-color: transparent; border: 2px solid #b29ae7; }
            ...
            # We remove the border from the old table so that it doesn't look like a "box within a box" when we include it.
            self.ui.table_clips.setStyleSheet(self.ui.table_clips.styleSheet() + " QTableWidget { border: none; background-color: transparent; }")
            
            # 4. We put all of this in place of the old table.
            parent_layout = self.ui.table_clips.parentWidget().layout()
            if parent_layout:
                idx = parent_layout.indexOf(self.ui.table_clips)
                parent_layout.removeWidget(self.ui.table_clips)
                
                # We place both elements (both the list and the grid) into our new circle
                lib_layout.addWidget(self.ui.table_clips)
                lib_layout.addWidget(self.grid_clips)
                
                # Place the circle itself into the main interface
                parent_layout.insertWidget(idx, self.library_container)
                
            # 5. Mode Switching Logic
            self.ui.table_clips.hide() # Hide the boring list by default
            ...
            # Secret Click for the Grid
            self.grid_clips.itemClicked.connect(self.on_grid_item_clicked)
        ...
        if hasattr(self.ui, 'settings_tabs'):
            ...
            self.btn_toggle_settings = QPushButton("Hide ⬇️")
            ...
            def toggle_settings():
                ...
                if self.settings_collapsed:
                    ...
                    self.ui.settings_tabs.setMaximumHeight(max(bar_h, 28) + 2)
                    self.btn_toggle_settings.setText("Show ⬆️")
                else:
                    # Expand the panel
                    self.ui.settings_tabs.setMaximumHeight(16777215)
                    self.btn_toggle_settings.setText("Hide ⬇️")
        ...
        try:
            ...
            if hasattr(self.ui, 'btn_browse'):
                self.ui.btn_browse.setStyleSheet(modern_button_style)
            
            if hasattr(self, 'btn_refresh'):
                self.btn_refresh.setStyleSheet(modern_button_style)
                
            if hasattr(self.ui, 'btn_about'):
                self.ui.btn_about.setStyleSheet(modern_button_style)
                
            if hasattr(self.ui, 'btn_update_check'):
                self.ui.btn_update_check.setStyleSheet(modern_button_style)
                
        except AttributeError as e:
            print(f"Error applying style to button: {e}")
        ...
        try:
            ...
            bottom_buttons_style = """
                QPushButton { background-color: #383838; color: #ffffff; border: 2px solid #444444; border-radius: 10px; font-weight: bold; font-size: 12px; padding: 8px 22px; }
            ...
            """
            start_btn_style = """
                QPushButton { background-color: #2e6b32; color: #ffffff; border: 2px solid #3e8e41; border-radius: 10px; font-weight: bold; font-size: 13px; padding: 8px 22px; }
            ...
            if hasattr(self.ui, 'btn_start'): self.ui.btn_start.setStyleSheet(start_btn_style)
            if hasattr(self.ui, 'btn_pause'): self.ui.btn_pause.setStyleSheet(bottom_buttons_style)
            if hasattr(self.ui, 'btn_cancel'): self.ui.btn_cancel.setStyleSheet(bottom_buttons_style)
            if hasattr(self.ui, 'btn_logs'): self.ui.btn_logs.setStyleSheet(bottom_buttons_style)
            ...
            # TOP ROW (Settings Text + Ready)
            top_row = qtw.QHBoxLayout()
            if hasattr(self.ui, 'label_short_summary'):
                self.ui.label_short_summary.setStyleSheet("color: #e0e0e0; font-size: 13px;")
                top_row.addWidget(self.ui.label_short_summary)
            ...
            for btn_name in buttons_queue:
                if hasattr(self.ui, btn_name):
                    ...
                    #Expanding will force them to divide the entire width into exactly 4 parts!
                    btn.setSizePolicy(qtw.QSizePolicy.Expanding, qtw.QSizePolicy.Fixed)
                    btn.setMinimumHeight(36) # Making them pleasantly plump in height
        ...
        # Using QStackedLayout (Deck of Cards)
        self.video_stack = QStackedLayout(self.video_wrapper)
        self.video_stack.setContentsMargins(2, 2, 2, 2)
    ...
    def eventFilter(self, source, event):
        ...
        if hasattr(self, 'video_wrapper') and source == self.video_wrapper and event.type() == QEvent.Type.Resize:
            if getattr(self, 'is_fullscreen', False) and hasattr(self, 'player_footer_frame'):
                # Dynamically calculate the control panel borders in the center of the screen
                w = self.video_wrapper.width()
                h = self.video_wrapper.height()
                footer_h = self.player_footer_frame.sizeHint().height()
                
                # Press the HUD panel to the bottom of the screen at a distance of 15px
                self.player_footer_frame.setGeometry(40, h - footer_h - 15, w - 80, footer_h)
                
                # APPLY A HARDWARE ROUNDING MASK (16px) 
                from PySide6.QtGui import QPainterPath, QRegion
                path = QPainterPath()
                # build a rounded contour exactly to the new panel dimensions
                path.addRoundedRect(0.0, 0.0, float(w - 80), float(footer_h), 16.0, 16.0)
    ...
    def close_current_clip(self):
        ...
        if hasattr(self.ui, 'video_container'):
            self.ui.video_container.setStyleSheet("background-color: black; border: none;")
    ...
    def toggle_fullscreen(self):
        ...
        if self.is_fullscreen:
            ...
            # Remember the window state BEFORE switching to Fullscreen
            self.window_maximized_before = self.ui.isMaximized()
            self.normal_geometry = self.ui.geometry()
            ...
            # 1. Hard-code the name so that the style is 100% applied
            self.player_footer_frame.setObjectName("HudFrame")
            
            # 2. Perfect translucent glass with rounding
            self.player_footer_frame.setStyleSheet("""
                QFrame#HudFrame { 
            ...
            # Smart Window Restore (Maximized or Geometry) ---

            self.ui.setWindowState(Qt.WindowState.WindowNoState)
            ...
            if getattr(self, 'window_maximized_before', False):
                ...
                if getattr(self, 'normal_geometry', None):
                    self.ui.setGeometry(self.normal_geometry)
            ...
            # Reset the hard stretch limits so that windowed mode Layout can manage the sizes
            self.player_footer_frame.setMinimumWidth(0)
            self.player_footer_frame.setMaximumWidth(16777215)
            self.player_footer_frame.setMinimumHeight(0)
            self.player_footer_frame.setMaximumHeight(16777215)

            # Move the HUD back inside the right panel
            ...
            # Return the normal color and shape without rendering bugs
            self.player_footer_frame.setObjectName("HudFrame")
            self.player_footer_frame.setStyleSheet("""
                QFrame#HudFrame { 
                    background-color: #2d2d2d; 
            ...
            # We ask the system to force update and stretch the form according to the grid
            self.player_footer_frame.updateGeometry()
            if right_layout:
    ...
    def generate_and_play_preview(self):
        ...
        # 2. If not found move up one level.
        if not json_path:
            parent1 = os.path.dirname(clip_path)
            print(f"JSON not found, take a look on 1 layer upper: {parent1}")
            json_path = find_json_in_dir(parent1)
            
        # 3. We're moving up 2 levels!
        if not json_path:
            parent2 = os.path.dirname(os.path.dirname(clip_path))
            print(f"JSON not found, take a look on 2 layers upper: {parent2}")
            json_path = find_json_in_dir(parent2)

        # 2. Pass to the engine and calculate the precise Offset.
        if hasattr(self, 'custom_timeline'):
            if json_path:
                print(f"Json was found successfullyyy: {json_path}")
                
                json_name = os.path.basename(json_path) 
                # Extract the video folder name from mpd_path.
                ...
                print(f"The JSON file was not found in the folder at all: {clip_path}")
                self.custom_timeline.canvas.markers.clear()
                self.custom_timeline.canvas.update()
...
<MPD xmlns="urn:mpeg:dash:schema:mpd:2011" profiles="urn:mpeg:dash:profile:isoff-live:2011" type="static" mediaPresentationDuration="PT{duration_sec}S">
  <Period id="0" start="PT0.000S">
    ...
    def on_grid_item_clicked(self, item):
        """ When we click on a card, we secretly click on a hidden table """
        hidden_row_idx = item.data(Qt.UserRole)
        if hasattr(self.ui, 'table_clips'):
            self.ui.table_clips.selectRow(hidden_row_idx)
            # Force-pushing a UI update.
            if hasattr(self, 'update_quality_options'):
                self.update_quality_options()

    def build_netflix_grid(self):
        """Transforms rows from a boring, hidden table into vibrant cards."""
        import PySide6.QtWidgets as qtw
        ...
        for row in range(self.ui.table_clips.rowCount()):
            ...
            if clip_path:
                ...
                # Extract the clip type (FG, BG, Clip) from the folder name.
                if len(parts) > 0:
            ...
            # Creating a card (Passing date and time together!)
            ...
            item.setData(Qt.UserRole, row) # Saving the Index for the Secret Click
    ...
    def update_final_setup(self):
        ...
        #Retrieve the game name from the table.
        game_name = "Steam Clip"
        if hasattr(self.ui, 'table_clips') and self.ui.table_clips.currentRow() >= 0:
            game_name = self.ui.table_clips.item(self.ui.table_clips.currentRow(), 0).text()
            
        # Creating the Icon
        game_icon = getattr(self, 'current_game_icon', '')
        img_tag = f'<img src="{game_icon}" width="16" height="16" align="middle">' if os.path.exists(game_icon) else '★'
        
        if audio_only:
            short_text = f"{img_tag} &nbsp;<b>{game_name}</b> &nbsp;•&nbsp; AUDIO ONLY: {audio_format} {audio_bitrate_clean}"
        elif mute_audio:
            short_text = f"{img_tag} &nbsp;<b>{game_name}</b> &nbsp;•&nbsp; {q_word}, {fps_display} &nbsp;•&nbsp; {video_bitrate_display} &nbsp;•&nbsp; {codec} (Muted)"
        else:
            short_text = f"{img_tag} &nbsp;<b>{game_name}</b> &nbsp;•&nbsp; {q_word}, {fps_display} &nbsp;•&nbsp; {video_bitrate_display} &nbsp;•&nbsp; {codec}"
...
class ClipCard(qtw.QWidget):
    def __init__(self, title, date_str, badge_text, thumb_path, icon_path, row_idx, parent=None):
        ...
        self.setStyleSheet("QWidget { background-color: #2a2a2a; border-radius: 12px; }")
        ...
        self.thumb_label.setStyleSheet("background-color: #1a1a1a; border-radius: 12px; border-bottom-left-radius: 0px; border-bottom-right-radius: 0px;")
        ...
        # 3. --- BADGE (FG / Kills) (OVERLAY) ---
        self.badge_label = qtw.QLabel(badge_text, self.thumb_label)
        self.badge_label.setStyleSheet("background-color: #b29ae7; color: black; font-family: 'Segoe UI'; font-weight: bold; font-size: 11px; border-radius: 4px; padding: 2px 6px;")
        self.badge_label.move(260 - 35, 146 - 25) 
        ...
        date_lbl.setStyleSheet("color: #888888; font-size: 11px;")
...
class TimelineCanvas(QWidget):
        ...
        try:
            ...
            for entry in entries:
               # Subtract the time of the cut pieces!
                ...
                # If the event occurred in chunks that Steam has already deleted, simply ignore it
                if time_ms < 0: 
                    continue 
                ...
                if type_ not in ['event', 'screenshot', 'error', 'restrict']: continue
    ...
    def get_icon_pixmap(self, icon_key, is_round):
        ...
        if is_round:
            ...
            result = QPixmap(total_width, 36) # 🔥 36px
    ...
    def paintEvent(self, event):
        ...
        def draw_marker(marker, is_hovered):
            ...
            if not marker['is_round']:
                ...
                painter.drawLine(conn_x, int(base_bottom), conn_x, int(track_y))
        ...
        # 2. TOP LAYER: Draw the hovered icon (on top of everything else!)
    ...
    def mousePressEvent(self, event):
        ...
        # CLICK ON THE ICON (Jump back 2 seconds)
        if getattr(self, 'hovered_marker', None):
            # Subtract 2000 ms (2 seconds) from the event time
            jump_time = max(0, self.hovered_marker['time_ms'] - 2000)
            self.force_jump(jump_time)
            return #We interrupt the function so that the player's normal seek doesn't work.
        ...
        if found_marker != getattr(self, 'hovered_marker', None):
            ...
            if hasattr(self, 'text_tooltip'):
                if found_marker:
                    ...
                    # Jump to the main container to determine its height.
                    scroll_area = self.parentWidget().parentWidget() if self.parentWidget() else self
                    # Set the Y-coordinate exactly 4 pixels below the scrollbar.
                    global_y = scroll_area.mapToGlobal(QPoint(0, scroll_area.height() + 4)).y()
                    # Center the X-coordinate directly beneath the icon.
...
class FullscreenEventFilter(QObject):
    ...
    def eventFilter(self, obj, event):
        ...
        # Hide the frozen preview when minimizing
        if event.type() == QEvent.Type.ApplicationStateChange:
            if QApplication.instance().applicationState() != Qt.ApplicationState.ApplicationActive:
                # If the application is minimized or loses focus, we kill the preview
...
class SpeedControlWidget(QWidget):
    """ Smart YouTube-style expandable speed control """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setFixedHeight(40)
        ...
        # State memory for the toggle (click to reset to 1.0x)
        self.previous_speed = 10 # 10 means 1.0x

        # Preload all 4 icons
        self.icon_05 = QIcon(get_resource_path("speed0.5.png")) if os.path.exists(get_resource_path("speed0.5.png")) else QIcon()
        self.icon_10 = QIcon(get_resource_path("speed1.0.png")) if os.path.exists(get_resource_path("speed1.0.png")) else QIcon()
        self.icon_20 = QIcon(get_resource_path("speed2.0.png")) if os.path.exists(get_resource_path("speed2.0.png")) else QIcon()
        self.icon_30 = QIcon(get_resource_path("speed3.0.png")) if os.path.exists(get_resource_path("speed3.0.png")) else QIcon()
        ...
        if not self.icon_10.isNull():
            self.btn_icon.setIcon(self.icon_10)
            self.btn_icon.setIconSize(QSize(24, 24))
        else:
            self.btn_icon.setText("1.0x") 

        # Bind the click event to our smart toggle function
        self.btn_icon.clicked.connect(self.toggle_speed)

        # 2. The Speed Slider 
        # Range: 5 to 30 (which translates to 0.5x to 3.0x)
        self.slider = QSlider(Qt.Horizontal, self)
        ...
        # We reuse the volume line image!
        line_path = get_resource_path("linevolume.png").replace("\\", "/")
        self.slider.setStyleSheet(f"""
            QSlider::groove:horizontal {{ height: 4px; border-image: url("{line_path}"); background: rgba(255, 255, 255, 50); border-radius: 2px; }}
            QSlider::sub-page:horizontal {{ background: #b498e3; border-radius: 2px; }}
        ...
        # 3. Percentage Text 
        self.lbl_percent = QLabel("1.0x", self)
        self.lbl_percent.setFixedSize(45, 20)
        self.lbl_percent.move(136, 10)
        self.lbl_percent.setStyleSheet("color: white; font-size: 11px; font-weight: bold;")
        self.lbl_percent.setAlignment(Qt.AlignCenter)
    ...
    def toggle_speed(self):
        ...
        if self.slider.value() == 10:
            restore_val = self.previous_speed if self.previous_speed != 10 else 20 # default to 2.0x if no history
            self.slider.setValue(restore_val)
        else:
    ...
    def update_text(self, val):
        """ Dynamically swaps text and icons based on speed (0.5 to 3.0) """
        speed = val / 10.0
        self.lbl_percent.setText(f"{speed:.1f}x")
        
        # Swap logic exactly as you requested!
        if speed < 1.0:
            if not self.icon_05.isNull(): self.btn_icon.setIcon(self.icon_05)
        elif speed < 2.0:
            if not self.icon_10.isNull(): self.btn_icon.setIcon(self.icon_10)
        elif speed < 3.0:
            if not self.icon_20.isNull(): self.btn_icon.setIcon(self.icon_20)
        else: # == 3.0
            if not self.icon_30.isNull(): self.btn_icon.setIcon(self.icon_30)

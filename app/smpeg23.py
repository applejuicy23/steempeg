...
class SteempegApp(QObject):
    def __init__(self):
        ...
        if hasattr(self.ui, 'table_clips'):
            ...
            # Sorting
            self.combo_sort = qtw.QComboBox()
            self.combo_sort.addItems(["Sorted: Newest ↓", "Sorted: Oldest ↑", "Game: A-Z"])
            self.combo_sort.setCursor(qtc.Qt.PointingHandCursor)
            self.combo_sort.setStyleSheet("""
                ...
                QComboBox QAbstractItemView {
                    ...
                    border-radius: 6px;
                    selection-background-color: #5138e6;
                }
            """)
            ...
            top_pill_layout.addStretch() # Push the sort to the right
            top_pill_layout.addWidget(self.combo_sort)
            ...
            self.grid_clips.setStyleSheet("""
                ...
                QListWidget::item { border-radius: 12px; }
                QListWidget::item:selected { border: 2px solid #b29ae7; background: transparent; }
        ...
        """)
        mega_layout = QHBoxLayout(mega_pill)
        mega_layout.setContentsMargins(4, 4, 4, 4)
        mega_layout.setSpacing(2)
        
        # 1. Move the "Browse" button to a new location.
        old_browse_btn = self.ui.btn_browse
        if old_browse_btn.parentWidget():
            old_browse_btn.parentWidget().layout().replaceWidget(old_browse_btn, mega_pill)
            
        # 2. Ripping About and Update out of their old locations
        btn_about = getattr(self.ui, 'btn_about', None)
        btn_update = getattr(self.ui, 'btn_update_check', None)
        
        if btn_about and btn_about.parentWidget(): btn_about.parentWidget().layout().removeWidget(btn_about)
        if btn_update and btn_update.parentWidget(): btn_update.parentWidget().layout().removeWidget(btn_update)

        # 3. Unified Style for Internal Transparent Buttons
        inner_style = """
            QPushButton { 
                background-color: transparent; 
                color: #dddddd; 
        ...
        old_browse_btn.setStyleSheet(inner_style)
        old_browse_btn.setCursor(Qt.PointingHandCursor)
        if btn_about:
            btn_about.setStyleSheet(inner_style)
            btn_about.setCursor(Qt.PointingHandCursor)
        if btn_update:
            btn_update.setStyleSheet(inner_style)
            btn_update.setCursor(Qt.PointingHandCursor)

        # 4. Creating an Accent Refresh
        ...
        self.btn_refresh.setStyleSheet("""
            ...
            QPushButton:hover { background-color: #634bf2; }
            QPushButton:pressed { background-color: #4028cc; }
        """)
        self.btn_refresh.clicked.connect(self.scan_clips)
        
        # 5. Packing everything into a capsule
        mega_layout.addWidget(old_browse_btn)
        if btn_about: mega_layout.addWidget(btn_about)
        if btn_update: mega_layout.addWidget(btn_update)
        mega_layout.addWidget(self.btn_refresh)
        ...
        try:
            ...
            # Logs 
            btn_logs_style = """
                QPushButton { background-color: #383838; color: #ffffff; border: 2px solid #444444; border-radius: 8px; font-weight: bold; font-size: 12px; padding: 6px 14px; }
            ...
            start_btn_style = """
                QPushButton { background-color: #2e6b32; color: #ffffff; border: 2px solid #3e8e41; border-radius: 8px; font-weight: bold; font-size: 12px; padding: 6px 14px; }
            ...
            btn_pause_style = """
                QPushButton { background-color: #8c7314; color: #ffffff; border: 2px solid #a88b11; border-radius: 8px; font-weight: bold; font-size: 12px; padding: 6px 14px; }
            ...
            btn_cancel_style = """
                QPushButton { background-color: #8a2525; color: #ffffff; border: 2px solid #a82e2e; border-radius: 8px; font-weight: bold; font-size: 12px; padding: 6px 14px; }
            ...
            if hasattr(self.ui, 'btn_pause'): self.ui.btn_pause.setStyleSheet(btn_pause_style)
            if hasattr(self.ui, 'btn_cancel'): self.ui.btn_cancel.setStyleSheet(btn_cancel_style)
            if hasattr(self.ui, 'btn_logs'): self.ui.btn_logs.setStyleSheet(btn_logs_style)
        ...
        if hasattr(self.ui, 'main_splitter'):
            self.ui.main_splitter.setSizes([300, 900])
    ...
    def scan_clips(self):
        ...
        try:
            for full_path in folders_to_check:
                ...
                if len(parts) >= 4 and parts[1].isdigit():
                    ...
                    try:
                        formatted_time = datetime.strptime(parts[3], "%H%M%S").strftime("%I:%M %p")
                    except:
                        formatted_time = ""
...
class TimelineCanvas(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        h = QImage(get_resource_path("scrollerhead.png"))

...
class SteempegApp:
    def __init__(self):
        ...
        loader = QUiLoader()
        ui_file_path = get_resource_path("smpegui9.ui")
        ...
        self.ui.setWindowTitle("Steempeg v7")
        ...
        logging.info("="*40)
        logging.info("STEEMPEG 7 RUNNING")
        logging.info("="*40)
        ...
        if hasattr(self.ui, 'btn_logs'):
            self.ui.btn_logs.clicked.connect(self.open_logs_folder)
        ...
        if hasattr(self, 'current_clip_duration_sec') and self.current_clip_duration_sec > 0:
            ...
            if "Target File Size" in quality:
                if hasattr(self, 'dynamic_stops') and hasattr(self.ui, 'size_slider'):
                    ...
                    if target_mb >= 1000:
                        size_str = f"~{target_mb / 1024:.2f} GB (Target)"
                    else:
                        size_str = f"~{target_mb} MB (Target)"
                else:
                    size_str = "Unknown"
    ...
    def start_render_thread(self):
        ...
        logging.info(f"Settings: Quality={quality_text}, FPS={fps_text}, Bitrate={video_bitrate}, Codec={selected_encoder}")
    ...
    def run(self):
        ...
        try:
            ...
            for idx, mpd in enumerate(self.mpd_paths):
                ...
                if "Original" in self.quality_text and "Target File" not in self.quality_text:
                    ...
                    cmd = f'"{self.ffmpeg_exe}" -i "{safe_mpd}" -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" {fps_arg}-c:v {self.selected_encoder} -b:v {self.video_bitrate} -c:a aac -b:a 128k -y "{temp_mp4}"'
                    ...
                    if match:
                        ...
                        cmd = f'"{self.ffmpeg_exe}" -i "{safe_mpd}" -vf scale=-2:{target_height} {fps_arg}-c:v {self.selected_encoder} -b:v {self.video_bitrate} -c:a aac -b:a {audio_bitrate} -y "{temp_mp4}"'
                    else:
                        cmd = f'"{self.ffmpeg_exe}" -i "{safe_mpd}" {fps_arg}-c copy -y "{temp_mp4}"'

...
class SteempegApp(QObject):
    ...
    def eventFilter(self, source, event):
        ...
        if hasattr(self, 'video_wrapper') and source == self.video_wrapper and event.type() == QEvent.Type.Resize:
            if getattr(self, 'is_fullscreen', False) and hasattr(self, 'player_footer_frame'):
                # Динамически вычисляем границы панели управления по центру экрана
                w = self.video_wrapper.width()
                h = self.video_wrapper.height()
                footer_h = self.player_footer_frame.sizeHint().height()
                
                # Прижимаем HUD панель к низу экрана на расстоянии 15px
                self.player_footer_frame.setGeometry(40, h - footer_h - 15, w - 80, footer_h)
                
                # === ПРИМЕНЯЕМ АППАРАТНУЮ МАСКУ СКРУГЛЕНИЯ (16px) ===
                from PySide6.QtGui import QPainterPath, QRegion
                path = QPainterPath()
                # Строим скругленный контур точно под новые размеры панели
                path.addRoundedRect(0.0, 0.0, float(w - 80), float(footer_h), 16.0, 16.0)
                region = QRegion(path.toFillPolygon().toPolygon())
                self.player_footer_frame.setMask(region) # Вырезаем острые углы навсегда!
    ...
    def toggle_fullscreen(self):
        ...
        if self.is_fullscreen:
            ...
            # 1. Жестко задаем имя, чтобы стиль 100% применился
            self.player_footer_frame.setObjectName("HudFrame")
            
            # 2. Идеальное полупрозрачное стекло со скруглением
            self.player_footer_frame.setStyleSheet("""
                QFrame#HudFrame { 
            ...
            # === ОЧИСТКА И ВОЗВРАТ ПАНЕЛИ В ИСХОДНЫЙ UI ===
            self.player_footer_frame.setParent(self.ui.right_panel)
            self.player_footer_frame.clearMask() # Полностью отключаем маску, в окне панель снова прямоугольник

            # Сбрасываем жесткие лимиты растяжения, чтобы Layout оконного режима мог управлять размерами
            ...
            # Пересыпаем HUD обратно внутрь правой панели
            ...
            # Возвращаем обычный цвет и форму без багов отрисовки
            self.player_footer_frame.setObjectName("HudFrame")
            self.player_footer_frame.setStyleSheet("""
                QFrame#HudFrame { 
                    background-color: #2d2d2d; 
            ...
            # Просим систему принудительно обновить и растянуть форму по сетке
...
# --- SMART PREVIEW SNIPER (RAM CACHE DASH ENGINE) ---
...
class PreviewSniperWorker(QThread):
    ...
    def __init__(self):
        ...
        self.cache = {} # Оперативный RAM-кэш
        self.interval = 3 
        self.current_process = None # Храним ссылку на активный FFmpeg подпроцесс

    def request_frame(self, video_path, hover_sec):
        ...
        # Если кадр уже в кэше — отдаем моментально без запуска процессов
        if target_sec in self.cache:
            self.preview_ready.emit(target_sec, self.cache[target_sec])
            return

        self.queue = [target_sec]
        
        # --- PRO KILL: Моментально убиваем устаревший процесс FFmpeg ---
        if self.current_process:
            try:
                import sys
                if sys.platform == "win32":
                    subprocess.call(['taskkill', '/F', '/T', '/PID', str(self.current_process.pid)], 
                                    stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) # Ошибка исправлена!
                else:
    ...
    def run(self):
        while self.queue and self.is_running:
            sec = self.queue.pop(0)
            if sec in self.cache:
                self.preview_ready.emit(sec, self.cache[sec])
                continue
            ...
            try:
                ...
                # Запускаем и сохраняем ссылку для прерывания
...
class CustomTimelineWidget(QWidget):
    ...
    def on_preview_ready(self, sec, pixmap):
        ...
        # Проверяем положение мыши прямо сейчас
        if getattr(self, 'is_hovering', False):
            hover_ms = max(0.0, min(self.x_to_ms(self.hover_x), float(self.duration_ms)))
            current_hover_sec = int(hover_ms // 1000)
            current_target_sec = round(current_hover_sec / 3.0) * 3
            
            # Обновляем картинку ТОЛЬКО если снайпер принес нужный секунде кадр!
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.is_trim_mode and not is_outside_trim_hitbox:
            ...
            if abs(x - start_x) <= hit_tolerance or abs(x - end_x) <= hit_tolerance:
                ...
                self.is_hovering_trim_handle = True
        ...
        # --- ULTRA-FAST FLOATING PREVIEW LOGIC ---
        if hasattr(self, 'preview_widget'):
            ...
            # Приоритет №1: Мгновенное чтение готового кадра с диска (без тормозов!)
            ...
            if not has_disk_thumb:
                ...
                if hasattr(self, 'sniper') and target_sec in self.sniper.cache:
                    ...
                    # Приоритет №3: Спокойный фоновый запуск без показа старых "призраков"
                    if hasattr(self, 'sniper') and self.current_video_path:
                        self.sniper.request_frame(self.current_video_path, sec)
                    
                    self.preview_widget.update_info(time_str, is_in_trim, hover_ms, None)
                    self.preview_widget.img_label.setPixmap(QPixmap())
                    self.preview_widget.img_label.setText("Generating...")

            # Координатное позиционирование превьюшки
        ...
        if hasattr(self, 'preview_widget'):
            ...
            # 1. Command the sniper to fetch THIS exact frame immediately!
            if hasattr(self, 'sniper') and self.current_video_path:
                self.sniper.request_frame(self.current_video_path, sec)

            # 2. Update UI and feed RAM image
            current_thumb_dir = getattr(self, 'thumb_dir', None)
            
            # Try RAM Cache first, fallback to Batch folder
            if self.current_preview_pixmap:
                self.preview_widget.update_info(time_str, is_in_trim, hover_ms, None)
                self.preview_widget.update_image_from_ram(self.current_preview_pixmap)
            else:
                self.preview_widget.update_info(time_str, is_in_trim, hover_ms, current_thumb_dir)
...
class FullscreenEventFilter(QObject):
    ...
    def eventFilter(self, obj, event):
        ...
        # --- ФИКС ALT+TAB: Прячем зависшую превьюшку при сворачивании ---
        if event.type() == QEvent.Type.ApplicationStateChange:
            if QApplication.instance().applicationState() != Qt.ApplicationState.ApplicationActive:
                # Если приложение свернуто или потеряло фокус - убиваем превьюшку

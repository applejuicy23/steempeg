...
class SteempegApp:
    def __init__(self):
        ...
        self.ui.setWindowTitle("Steempeg v11")
        ...
        logging.info("="*40)
        logging.info("STEEMPEG 11 RUNNING")
        logging.info("="*40)
    ...
    def skip_backward(self):
        ...
        if hasattr(self, 'player') and self.player:
            ...
            if state == vlc.State.Ended:
                ...
                if length > 10000:
                    self.player.set_time(length - 10000)
                self.ui.btn_play.setText("⏸ Pause")
                return
    ...
    def skip_forward(self):
        ...
        if hasattr(self, 'player') and self.player:
            state = self.player.get_state()
            if state == vlc.State.Ended:
                return # If you're already at the end, there's nowhere to skip ahead
            
            current_time = self.player.get_time()
            length = self.player.get_length()
            if current_time >= 0 and length > 0:
                self.player.set_time(min(length, current_time + 10000))
    ...
    def show_about_dialog(self):
        ...
        about_text = f"""
        <h3>Steempeg v11.0</h3>
        <p><b>Build:</b> 11.0 (Next-Gen)</p>
            ...
            if response.status_code == 200:
                ...
                if match:
                    ...
                    if latest_version > CURRENT_VERSION:
                        ...
                        # We look for the format in which the developer released the update (.exe or .zip)
                        download_url = None
                        asset_name = None
                        is_zip_update = False
                        ...
                        for asset in data.get("assets", []):
                            ...
                            if name.endswith(".exe"):
                                ...
                                is_zip_update = True
                                download_url = release_url # Link to the release page itself
                                break
                        ...
                        if is_zip_update:
                            msg.setText(f"<h3>Major Update!</h3><p>Version <b>{latest_name}</b> uses a new high-performance architecture.</p><p>Please download the .zip archive manually from GitHub.</p>")
                            btn_download = msg.addButton("🌐 Open GitHub", QMessageBox.ActionRole)
                        else:
                        ...
                        if msg.clickedButton() == btn_download:
                            if is_zip_update or not getattr(sys, 'frozen', False):
                                webbrowser.open(release_url) # Open a browser if it's a ZIP or we're testing in an IDE
                            elif download_url:
    ...
    def on_update_downloaded(self, success, filepath, final_asset_name):
        ...
        if success:
            ...
            # If the names happen to be the same
    ...
    def update_quality_options(self):
        ...
        # --- PLAYER HEADER DATA ---
        # Extract data from the table for your future beautiful UI header
        game_name = self.ui.table_clips.item(selected_row, 0).text()
        clip_date = self.ui.table_clips.item(selected_row, 2).text()
        clip_time = self.ui.table_clips.item(selected_row, 3).text()
        
        header_text = f"{game_name}   •   {clip_date}   •   {clip_time}"
        
        # If you create a QLabel named 'label_player_header' in Qt Designer, it will automatically show up!
        if hasattr(self.ui, 'label_player_header'):
            self.ui.label_player_header.setText(header_text)

...
class SteempegApp:
    def __init__(self):
        ...
        loader = QUiLoader()
        ui_file_path = get_resource_path("smpegui10.ui")
        ...
        self.ui.setWindowTitle("Steempeg v8")
        ...
        logging.info("="*40)
        logging.info("STEEMPEG 8 RUNNING")
        logging.info("="*40)
    ...
    def show_about_dialog(self):
        """ Shows the About dialog with developer info """
        ...
        about_text = (
            "<h3>Steempeg v8.0</h3>"
            "<p><b>Build:</b> 8.0.1 (Stable)</p>"
            "<p><b>Developer:</b> Emily 🎀</p>"
        ...
        # Use a standard button to avoid double clicks
            ...
            if response.status_code == 200:
                ...
                if match:
                    ...
                    if latest_version > CURRENT_VERSION:
                        ...
                        btn_download = msg.addButton("🚀 Download Update", QMessageBox.ActionRole)
    ...
    def on_audio_only_toggled(self, checked):
        ...
        if hasattr(self.ui, 'tab_video'): self.ui.tab_video.setEnabled(not checked) 
        self.update_final_setup()
    ...
    def on_mute_audio_toggled(self, checked):
        ...
        if hasattr(self.ui, 'frame_audio_settings'): self.ui.frame_audio_settings.setEnabled(not checked)
        self.update_final_setup()
    ...
    def update_final_setup(self):
        ...
        #5. Beautiful detailed summary in Export Settings
        ...
        if audio_only:
            short_text = f"AUDIO ONLY: {audio_format} - {final_filename}"
        elif mute_audio:
            short_text = f"{q_word}, {fps_word}FPS, {codec}, {enc_word} (MUTED) - {final_filename}"
        else:
            short_text = f"{q_word}, {fps_word}FPS, {codec}, {enc_word} - {final_filename}"
    ...
    def setup_dynamic_slider(self):
        ...
        # Connect the recalculation function to the slider movement
        try: self.ui.size_slider.valueChanged.disconnect() 
        except: pass
...
if __name__ == "__main__":
    ...
    try:
        ...
        if getattr(window, 'ui', None) is None:
            QMessageBox.critical(None, "Interface Error", "Failed to load smpegui10.ui! Check if the file is located next to the script and that the name matches.")
            sys.exit(1)
